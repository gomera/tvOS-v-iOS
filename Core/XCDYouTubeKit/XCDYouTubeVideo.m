...
NSDictionary *XCDDictionaryWithQueryString(NSString *string, NSStringEncoding encoding)
{
	...
	for (NSString *field in fields)
	{
		...
		if (pair.count == 2)
		{
            ...
            NSString* value = (NSString*)CFBridgingRelease(CFURLCreateStringByReplacingPercentEscapes(
                                                                                                     kCFAllocatorDefault,
                                                                                                     (CFStringRef)pair[1],
                                                                                                     CFSTR("")));
	...
	if (streamMap.length > 0 || httpLiveStream.length > 0)
	{
		...
		for (NSString *streamQuery in streamQueries)
		{
			...
			if (urlString && itag)
			{
				...
				if (signature) {
					NSData *data = [[NSString stringWithFormat:@"%@&signature=%@", urlString, signature] dataUsingEncoding:NSUTF8StringEncoding];
					streamURL = [NSURL URLWithDataRepresentation:data relativeToURL:nil];
                    NSLog(@"%@",[streamURL description]);
				}
