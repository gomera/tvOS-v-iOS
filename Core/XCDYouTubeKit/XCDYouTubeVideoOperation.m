...
- (void) startNextRequest
{
	...
	{
		XCDYouTubeRequestType requestType = [objc_getAssociatedObject(self.dataTask, XCDYouTubeRequestTypeKey) unsignedIntegerValue];
...
- (void) startRequestWithURL:(NSURL *)url type:(XCDYouTubeRequestType)requestType
{
			...
			switch (requestType)
				{
					...
					{
						NSData *d = data != nil ? data : [NSData new];
						NSString *videoQuery = [[NSString alloc] initWithData:d encoding:NSASCIIStringEncoding];
					...
					case XCDYouTubeRequestTypeWatchPage:
						[self handleWebPageResponseWithData:data];
						break;
					case XCDYouTubeRequestTypeEmbedPage:
						[self handleEmbedWebPageResponseWithData:data];
						break;
					case XCDYouTubeRequestTypeJavaScriptPlayer:
						[self handleJavaScriptPlayerResponseWithData:data];
						break;
				}
		} else {
	...
	objc_setAssociatedObject(self.dataTask, XCDYouTubeRequestTypeKey, @(requestType), OBJC_ASSOCIATION_RETAIN);
...
- (void) handleWebPageResponseWithData:(NSData*)data
{
	XCDYouTubeLogDebug(@"Handling web page response");
	
	self.webpage = [[XCDYouTubeVideoWebpage alloc] initWithData:data response:self.response];
...
- (void) handleEmbedWebPageResponseWithData:(NSData*)data
{
	XCDYouTubeLogDebug(@"Handling embed web page response");
	
	self.embedWebpage = [[XCDYouTubeVideoWebpage alloc] initWithData:data response:self.response];
...
- (void) handleJavaScriptPlayerResponseWithData:(NSData*)data
{
